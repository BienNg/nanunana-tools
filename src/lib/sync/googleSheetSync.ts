...
function analyzeRawSheetSessionSkips(
  ...
  for (let i = headerRowIndex + 1; i < rows.length; i++) {
    ...
    const reason = classifySessionDateSkip(parsedDate, rawDate, colIndices.datum !== -1, now);
    if (reason === 'future') {
...
async function syncOneCourseSheet(
  ...
  for (let i = headerRowIndex + 1; i < rows.length; i++) {
    ...
    sessionDrafts.push({
      ...
      datumRaw: String(row[colIndices.datum] ?? ''),
      parsedDate,
      startTime,
      endTime,
      teacherCell,
  ...
  for (const sess of sessionDrafts) {
    ...
    const dateSkipReason = classifySessionDateSkip(
      sess.parsedDate,
      sess.datumRaw,
      colIndices.datum !== -1,
      sessionSyncNow
