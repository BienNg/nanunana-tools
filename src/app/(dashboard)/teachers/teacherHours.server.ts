...
export function getLastThreeMonthBuckets() {
  const now = new Date();
...
export async function getTeachersWithHoursByStatus(statusFilter: StatusFilter): Promise<TeacherWithHours[]> {
  ...
  const lessonsByCourseId: Record<string, LessonRow[]> = {};
  ...
  const monthBuckets = getLastThreeMonthBuckets();
  ...
  return teacherList.map((teacher) => {
    ...
    const status = teacher.status === 'inactive' ? ('inactive' as const) : ('active' as const);
    return { id: teacher.id, name: teacher.name, status, monthHours };
  });
}
