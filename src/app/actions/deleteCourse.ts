...
export async function deleteCourseAndOrphans(courseId: string) {
  ...
  if (deleteCourseError) {
    return { ok: false as const, error: deleteCourseError.message };
  }
  ...
  const [{ data: allTeachers, error: allTeachersError }, { data: linkedTeacherRows, error: linkedTeacherRowsError }] =
    await Promise.all([supabase.from('teachers').select('id'), supabase.from('course_teachers').select('teacher_id')]);
  ...
  const orphanTeacherIds = (allTeachers ?? []).map((teacher) => teacher.id).filter((teacherId) => !linkedTeacherIds.has(teacherId));
  ...
  const [{ data: allStudents, error: allStudentsError }, { data: linkedStudentRows, error: linkedStudentRowsError }] =
    await Promise.all([supabase.from('students').select('id'), supabase.from('course_students').select('student_id')]);
  ...
  const orphanStudentIds = (allStudents ?? []).map((student) => student.id).filter((studentId) => !linkedStudentIds.has(studentId));
